...
ggplot(cumulative_data, aes(x = month_year, y = cumulative_funding, color = prog_lumped)) +
...
```{r}
p <- cumulative_data_area %>% 
  ggplot(aes(month_year,cumulative_funding, fill = prog_lumped)) +
  ...
  labs(
    ...
    subtitle = "Visualizing the long-tail fragmentation of capital allocation across all programmes",
    x = "Date",
    y = "Total Cumulative Commitment (Millions)",
    fill = "Programme"
  ) +
  theme(
    # Angle the dates to 45 degrees so they do not overlap
...
```{r}
# Save the plot with LinkedIn-optimized dimensions
ggsave(
  filename = "sfi_market_dominance_stacked_area.png", 
  plot = p,
  width = 7, 
  height = 4,
  units = "in",
